...
(function(window,undefined){
	...
	foxJS.__init = function(){ //初始化方法，用于导入文件后对全局的FOXJS对象初始化
		...
		this.importOnce = function(src){//导入一次
			scriptObj = document.getElementById(src);
			if (!scriptObj) {//如果没有导入
				this.import(src);
			}
			return src;
	...
	foxBaby = function(element){	//特殊的foxjs element对象
		...
		this.offset.innerX = this.element.clientLeft;
		this.offset.innery = this.element.clientTop;
